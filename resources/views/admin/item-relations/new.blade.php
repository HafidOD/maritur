@php
	$ajaxLinks = [
		1=>'/admin/hotels/search',
		2=>'/admin/tours/search',
		3=>'/admin/destinations/search',
	];
@endphp
<div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
    <h3 class='modal-title'>Agregar elemento a lista</h3>
</div>
<div class="modal-body">
<form action="{{url('admin/item-relations')}}" method="post" class="validate-form ajax-submit" data-close-modal-on-success='true' data-reload-tables='true'>
	{{ csrf_field() }}
	...
	<div class="form-group">
		...
		<input type="text" data-ajax='{{$ajaxLinks[$model->itemList->itemType]}}' class="form-control autocomplete-ajax" value="{{$model->referenceId>0?$model->referenceModel->name:''}}">
		<input type="hidden" name="model[referenceId]" value="<?= $model->referenceId ?>" class='required'>
	</div>
	@if ($model->itemList->itemType!=3)
		<div class="form-group">
			<label>Precio</label>
