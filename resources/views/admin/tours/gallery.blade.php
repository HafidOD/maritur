<div class="modal-header">
	<h3 class='modal-title'>Galería de {{ $tour->name }}</h3>
</div>
<div class="modal-body">
	@if (count($tour->getImages())>0)
	<div id='testsort' class="sortable-images row" data-update-positions-url='{{ url('admin/uploads/update-positions') }}'>
		@foreach ($tour->getImages() as $img)
			<div class="col-xs-3" data-img='{{ $img->id }}'>
				<a href="{{ AppImg::url($img) }}" target="_blank">
					<img src="{{ AppImg::url($img,150,150) }}">
				</a>
				<br>
				<br>
	...
	<div class="row">
		<div class="col-xs-4">
			<form data-reload-modal-on-success='true' class='ajax-submit validate-form' method='post' action='{{ url('admin/uploads/add') }}'>
			{{ csrf_field() }}
			<input type="hidden" name="referenceId" value="{{$tour->id}}">
			<input type="hidden" name="folder" value="tours">
